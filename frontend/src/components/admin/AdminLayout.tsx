...
export default function AdminLayout() {
  ...
  const navLinks = isCounsellor
    ? allNavLinks.filter((l) => l.to === '/admin/crm' || l.to === '/admin/enquiries')
    : allNavLinks
      ...
      <header className="bg-slate-950 border-b border-slate-800 sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Link to={isCounsellor ? '/admin/crm' : '/admin'} className="flex items-center gap-2">
            ...
            <span className="hidden sm:inline-block px-2.5 py-0.5 rounded-full text-[10px] font-extrabold uppercase bg-purple-950/80 text-purple-300 border border-purple-800">
              {isCounsellor ? 'CRM & Admissions' : 'Master In Tech Core'}
            </span>
          </div>
